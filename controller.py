    ...
    def decision(self):
        if self.flight_state_ == self.FlightState.WAITING:  # 起飞并飞至离墙体（y = 3.0m）适当距离的位置
            ...
            if abs(dist) < 0.3:  # 当前段导航结束
                ...
                command_m = [['right','left'],['forward','back'],['up','down']]
                command = command_m[dim_index][dir_index]
                # end of TODO 2
                if abs(dist) > 1.5:
            ...
            ####################################################################################################################
            if(self.count_init<2):
                self.publishCommand('forward 75')
                self.count_init = self.count_init + 1
            ####################################################################################################################
